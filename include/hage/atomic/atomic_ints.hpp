...
template<detail::AtomicIntegral T>
struct atomic<T>
{
private:
...
public:
  ...
  {
    return m_atomic.fetch_and(arg, order);
  }
  T fetch_and(T arg, std::memory_order order = std::memory_order::seq_cst) volatile noexcept
  {
    return m_atomic.fetch_and(arg, order);
  }
  ...
  {
    return m_atomic.fetch_or(arg, order);
  }
  T fetch_or(T arg, std::memory_order order = std::memory_order::seq_cst) volatile noexcept
  {
    return m_atomic.fetch_or(arg, order);
  }
  ...
  {
    return m_atomic.fetch_xor(arg, order);
  }
  T fetch_xor(T arg, std::memory_order order = std::memory_order::seq_cst) volatile noexcept
  {
    return m_atomic.fetch_xor(arg, order);
  }
  ...
  T operator++() noexcept { return m_atomic.operator++(); }
  T operator++() volatile noexcept { return m_atomic.operator++(); }
